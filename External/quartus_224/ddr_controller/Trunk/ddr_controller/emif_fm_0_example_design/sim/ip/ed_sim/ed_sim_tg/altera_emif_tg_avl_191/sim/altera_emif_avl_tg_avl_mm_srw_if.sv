...
module altera_emif_avl_tg_avl_mm_srw_if # (
   ...
   logic                          qdr4_delay_read_req;
   logic [7:0]                    qdr4_write_cnt;
   ...
   begin
      if (!reset_n)
         qdr4_write_cnt <= '0;
      else if (avl_ready_w & avl_write_req & ~(avl_ready & avl_read_req)) begin
          if (~qdr4_write_cnt[7])
            qdr4_write_cnt <= qdr4_write_cnt[6:0] + 7'b1;
      end
      else if (~(avl_ready_w & avl_write_req) & (avl_ready & avl_read_req)) begin
          qdr4_write_cnt <= qdr4_write_cnt - 7'b1;
      end
   end

   generate
       if (PROTOCOL_ENUM == "PROTOCOL_QDR4") begin: qdr4_read_delay
            assign qdr4_delay_read_req = ~|qdr4_write_cnt;
       end else begin: non_read_delay
            assign qdr4_delay_read_req = 1'b0;
       end
   endgenerate
   ...
   altera_emif_avl_tg_scfifo_wrapper # (
      ...
      .read_req       (can_issue_avl_r_cmd & ~fifo_r_empty & ~qdr4_delay_read_req),
   ...
   begin
      ...
      begin
         ...
         // Otherwise, all Avalon signals should be held constant.
         if (fifo_r_empty | qdr4_delay_read_req)
         begin
            avl_read_req   <= 1'b0;
         end
         else
