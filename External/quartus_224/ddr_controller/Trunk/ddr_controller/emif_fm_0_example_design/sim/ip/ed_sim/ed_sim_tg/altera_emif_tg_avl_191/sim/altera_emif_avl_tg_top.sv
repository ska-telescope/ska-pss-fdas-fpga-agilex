...
module altera_emif_avl_tg_top # (
   ...
   // Use simulation memory model validation driver
   parameter USE_SIM_MEM_VALIDATION_TG               = 0,
   parameter DIAG_SIM_MEMORY_PRELOAD_PRI_EMIF_FILE   = "",
   parameter DIAG_SIM_MEMORY_PRELOAD_SEC_EMIF_FILE   = "",
   ...
   input  logic                                               ctrl_ecc_readdataerror_1,
   ...
   `ifdef DISABLE_SIM_PARAMS_FOR_SYNTH
      localparam USE_SIM_MEM_VALIDATION_TG_AFT_SYNTH_OVRD   = 0;
   `else
      localparam USE_SIM_MEM_VALIDATION_TG_AFT_SYNTH_OVRD   = USE_SIM_MEM_VALIDATION_TG;
   `endif
   ...
   logic [7:0]                                   ctrl_ecc_user_interrupt_all;
   logic [7:0]                                   ctrl_ecc_readdataerror_all;
   ...
   generate
      if ( MEGAFUNC_DEVICE_FAMILY == "ARRIA 10")
      begin : gen_a10_reset
         always_ff @(posedge emif_usr_clk, negedge emif_usr_reset_n)
         begin : gen_emif_usr_reset_n_ff
            if (!emif_usr_reset_n) begin : gen_rst
               local_reset_n <= 1'b0;
            end else begin : gen_non_rst
               local_reset_n <= 1'b1;
            end
         end

         always_ff @(posedge emif_usr_clk_sec, negedge emif_usr_reset_n_sec)
         begin : gen_emif_usr_reset_n_sec_ff
            if (!emif_usr_reset_n_sec) begin : gen_rst
      ...
      begin : gen_ninit_reset_sync

         logic       local_reset_n_int;
         logic [8:0] local_reset_n_reg_chain;

         logic       local_reset_n_sec_int;
         logic [8:0] local_reset_n_sec_reg_chain;

         wire        ninit_done_int;
         assign      ninit_done_int = ~ninit_done;

         // Pipeline, synchronize and duplicate the emif_usr_reset_n signal for timing
         altera_std_synchronizer_nocut # (
            .depth     (DATA_SYNC_LENGTH),
            .rst_value (0)
         ) local_reset_n_sync_inst (
            .clk     (emif_usr_clk),
            .reset_n (emif_usr_reset_n),
            .din     (ninit_done_int),
            .dout    (local_reset_n_int)
         );

         // Pipeline, synchronize and duplicate the emif_usr_reset_n_sec signal for timing
         altera_std_synchronizer_nocut # (
            .depth     (DATA_SYNC_LENGTH),
            .rst_value (0)
         ) local_reset_n_sec_sync_inst (
            .clk     (emif_usr_clk_sec),
            .reset_n (emif_usr_reset_n_sec),
            .din     (ninit_done_int),
            .dout    (local_reset_n_sec_int)
         );

         always_ff @(posedge emif_usr_clk)
         begin : gen_reset_sync_clk_ff
            local_reset_n_reg_chain[8:0] <= {local_reset_n_reg_chain[7:0], local_reset_n_int};
            local_reset_n                <= local_reset_n_reg_chain[8];
         end

         always_ff @(posedge emif_usr_clk_sec)
         begin : gen_reset_sync_clk_sec_ff
            local_reset_n_sec_reg_chain[8:0] <= {local_reset_n_sec_reg_chain[7:0], local_reset_n_sec_int};
            local_reset_n_sec                <= local_reset_n_sec_reg_chain[8];
         end
      end
   endgenerate
...
`endif
   ...
   assign ctrl_ecc_user_interrupt_all  = {1'b0,                1'b0,                1'b0,                1'b0,                1'b0,                1'b0,                ctrl_ecc_user_interrupt_1, ctrl_ecc_user_interrupt_0};
   assign ctrl_ecc_readdataerror_all   = {1'b0,                1'b0,                1'b0,                1'b0,                1'b0,                1'b0,                ctrl_ecc_readdataerror_1,  ctrl_ecc_readdataerror_0};
      ...
      if (SEPARATE_READ_WRITE_IFS) begin : srw
            ...
            begin : gen_avl_mm_driver
               if (USE_SIMPLE_TG) begin : simple
                  ...
                  altera_emif_avl_tg_driver # (
                     ...
                     .avl_be                                 (amm_byteenable_all[2*i]),
                     .avl_wdata                              (amm_writedata_all[2*i]),
                     .ctrl_ecc_user_interrupt                (1'b0),
                     .ctrl_ecc_readdataerror                 (1'b0),
                     .pass                                   (traffic_gen_pass_all[(2*i)+1]),
                     .fail                                   (traffic_gen_fail_all[(2*i)+1]),
                     .timeout                                (traffic_gen_timeout_all[(2*i)+1]),
                     .pnf_per_bit                            (),
         ...
         begin : gen_avl_mm_driver
            if (i < NUM_OF_CTRL_PORTS) begin
               if (USE_SIM_MEM_VALIDATION_TG_AFT_SYNTH_OVRD) begin : sim_mem_validate
                  altera_emif_avl_tg_driver_sim_mem_validate # (
                     .DEVICE_FAMILY                          (MEGAFUNC_DEVICE_FAMILY),
                     .PROTOCOL_ENUM                          (PROTOCOL_ENUM),
                     .TG_AVL_ADDR_WIDTH                      (PORT_CTRL_AMM_ADDRESS_WIDTH),
                     .TG_AVL_WORD_ADDR_WIDTH                 (AMM_WORD_ADDRESS_WIDTH),
                     ...
                     .AMM_WORD_ADDRESS_DIVISIBLE_BY          (AMM_WORD_ADDRESS_DIVISIBLE_BY),
                     .AMM_BURST_COUNT_DIVISIBLE_BY           (AMM_WORD_ADDRESS_DIVISIBLE_BY),
                     .SIM_MEMORY_PRELOAD_EMIF_FILE           ((PHY_PING_PONG_EN && (i == 1)) ? DIAG_SIM_MEMORY_PRELOAD_SEC_EMIF_FILE : DIAG_SIM_MEMORY_PRELOAD_PRI_EMIF_FILE)
                  ) inst (
               ...
               end else if (USE_SIMPLE_TG) begin : simple
                  altera_emif_avl_tg_driver_simple # (
                     .DEVICE_FAMILY                          (MEGAFUNC_DEVICE_FAMILY),
                     .PROTOCOL_ENUM                          (PROTOCOL_ENUM),
                     .TG_TEST_DURATION                       (TEST_DURATION),
                  ...
                  altera_emif_avl_tg_driver # (
                     ...
                     .avl_addr_w                             (),     // unused
                     .avl_size_w                             (),     // unused
                     .ctrl_ecc_user_interrupt                (ctrl_ecc_user_interrupt_all[i]),
                     .ctrl_ecc_readdataerror                 (ctrl_ecc_readdataerror_all[i]),
                     .pass                                   (traffic_gen_pass_all[i]),
                     .fail                                   (traffic_gen_fail_all[i]),
                     .timeout                                (traffic_gen_timeout_all[i]),
                     .pnf_per_bit                            (),
