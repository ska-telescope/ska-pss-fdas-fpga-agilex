...
################################################################
proc FDAS_DDR_CONTROLLER_HPS_altera_emif_arch_fm_191_d4xre5a_add_report_timing_analysis {opcname inst var_array_name summary_name title from_clks to_clks from_nodes to_nodes } {
...
proc FDAS_DDR_CONTROLLER_HPS_altera_emif_arch_fm_191_d4xre5a_add_c2p_p2c_report_timing_analysis {opcname inst pin_array_name var_array_name summary_name title from_clks to_clks from_nodes to_nodes p2c} {
...
proc FDAS_DDR_CONTROLLER_HPS_altera_emif_arch_fm_191_d4xre5a_perform_core_analysis {opcname inst pin_array_name var_array_name summary_name} {
   ...
   if {$::io_only_analysis == 1} {
      ...
      set coreclkname [list ${master_instname}_core_usr_* ${master_instname}_core_afi_* ${master_instname}_core_dft_* ${master_instname}_ref_clock ${master_instname}_core_nios_clk [FDAS_DDR_CONTROLLER_HPS_altera_emif_arch_fm_191_d4xre5a_get_clock_name_from_pin_name $pins(pll_ref_clock)]]
      ...
      # Core-to-periphery
      set res [FDAS_DDR_CONTROLLER_HPS_altera_emif_arch_fm_191_d4xre5a_add_c2p_p2c_report_timing_analysis $opcname $inst $pin_array_name var global_summary "Core To Periphery" $coreclks $phyclks "*" $emif_regs 0]
      ...
      # Periphery-to-core
      set res [FDAS_DDR_CONTROLLER_HPS_altera_emif_arch_fm_191_d4xre5a_add_c2p_p2c_report_timing_analysis $opcname $inst $pin_array_name var global_summary "Periphery To Core" $phyclks $coreclks $emif_regs "*" 1]
      set setup_margin    [min $setup_margin    [lindex $res 0]]
      set hold_margin     [min $hold_margin     [lindex $res 1]]
      set recovery_margin [min $recovery_margin [lindex $res 2]]
      set removal_margin  [min $removal_margin  [lindex $res 3]]
      ...
      set res [FDAS_DDR_CONTROLLER_HPS_altera_emif_arch_fm_191_d4xre5a_add_report_timing_analysis $opcname $inst var global_summary "Within Core" $coreclks $coreclks $emif_regs $emif_regs]
      set setup_margin    [min $setup_margin    [lindex $res 0]]
      set hold_margin     [min $hold_margin     [lindex $res 1]]
      set recovery_margin [min $recovery_margin [lindex $res 2]]
      set removal_margin  [min $removal_margin  [lindex $res 3]]
      ...
      set res [FDAS_DDR_CONTROLLER_HPS_altera_emif_arch_fm_191_d4xre5a_add_report_timing_analysis $opcname $inst var global_summary "IP to User Logic" "*" "*" $emif_regs $rest_regs]
      set setup_margin    [min $setup_margin    [lindex $res 0]]
      set hold_margin     [min $hold_margin     [lindex $res 1]]
      set recovery_margin [min $recovery_margin [lindex $res 2]]
      set removal_margin  [min $removal_margin  [lindex $res 3]]
      ...
      set res [FDAS_DDR_CONTROLLER_HPS_altera_emif_arch_fm_191_d4xre5a_add_report_timing_analysis $opcname $inst var global_summary "User Logic to IP" "*" "*" $rest_regs $emif_regs]
      set setup_margin    [min $setup_margin    [lindex $res 0]]
      set hold_margin     [min $hold_margin     [lindex $res 1]]
      set recovery_margin [min $recovery_margin [lindex $res 2]]
      set removal_margin  [min $removal_margin  [lindex $res 3]]
      ...
      if {$var(DIAG_TIMING_REGTEST_MODE)} {
         set res [FDAS_DDR_CONTROLLER_HPS_altera_emif_arch_fm_191_d4xre5a_add_report_timing_analysis $opcname $inst var global_summary "Within User Logic" $coreclks $coreclks $rest_regs $rest_regs]
         set setup_margin    [min $setup_margin    [lindex $res 0]]
         set hold_margin     [min $hold_margin     [lindex $res 1]]
         set recovery_margin [min $recovery_margin [lindex $res 2]]
         set removal_margin  [min $removal_margin  [lindex $res 3]]
