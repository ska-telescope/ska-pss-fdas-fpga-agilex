...
# ----------------------------------------
# ACDS 22.2 94 win32 2022.07.19.13:16:01
...
set LD_LIBRARY_PATH [dict merge $LD_LIBRARY_PATH [dict get [fp_add::get_env_variables $SIMULATOR_TOOL_BITNESS] "LD_LIBRARY_PATH"]]
if {[dict size $LD_LIBRARY_PATH] !=0 } {
  set LD_LIBRARY_PATH [subst [join [dict keys $LD_LIBRARY_PATH] ":"]]
  setenv LD_LIBRARY_PATH "$LD_LIBRARY_PATH"
}
append ELAB_OPTIONS [subst [fp_add::get_elab_options $SIMULATOR_TOOL_BITNESS]]
append SIM_OPTIONS [subst [fp_add::get_sim_options $SIMULATOR_TOOL_BITNESS]]
...
alias file_copy {
  ...
  set memory_files [concat $memory_files [fp_add::get_memory_files "$QSYS_SIMDIR"]]
  ...
  set normalized_src [fp_add::normalize_path "$file"]
  set normalized_dest [fp_add::normalize_path "$dest_file"]
...
set design_libraries [dict merge $design_libraries [fp_add::get_design_libraries]]
...
alias com {
  ...
  set design_files [dict merge [fp_add::get_common_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
  set common_design_files [dict values $design_files]
  foreach file $common_design_files {
    eval $file
  }
  set design_files [list]
  set design_files [concat $design_files [fp_add::get_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
