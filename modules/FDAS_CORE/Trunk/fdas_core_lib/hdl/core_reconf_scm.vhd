...
--
-- Source Path:  fdas_core_lib/hdl/core_reconf_scm.vhd
--
-- Created:
--          by - droogm (COVNETICSDT17)
--          at - 13:20:53 13/06/2023
--
...
architecture scm of core_reconf is
  ...
  signal rst_msix_n_s     : std_logic;
  signal rst_msix_sys_n_s : std_logic;
  signal ddr1_wr_en_s     : std_logic;
  signal ddr1_rd_en_s     : std_logic;
  ...
  for all : msix use entity msix_lib.msix;
...
begin
  ...
  cld_i : cld
    generic map (
      ddr_g                    => 1,                --number of DDR Interfaces
  ...
  conv_i : conv
    generic map (
      ...
      res_pages_g      => res_pages_g            --Address bits for result store
    )
    port map (
      CLK_MC        => CLK_MC,
      CLK_SYS       => CLK_SYS,
      ...
      DDR_VALIDOUT  => ddr1_wr_en_s,
      MCDATAOUT     => mcdataout_conv_s,
      READYOUT      => conv_ready_s,
      IFFT_LOOP_NUM => ifft_loop_num_s(ifft_loop_bits_g downto 0),
      SEGDONEOUT    => open,
  ...
  ctrl_i : ctrl
    port map (
      ...
      conv_waitreq   => ddr1_wr_waitreq,
      conv_wr_en     => ddr1_wr_en_s,
      hsum_done      => hsum_done_s,
      hsum_rd_en     => ddr1_rd_en_s,
      hsum_valid     => ddr1_rd_valid,
      hsum_waitreq   => ddr1_rd_waitreq,
      mcaddr         => mcaddr(18 downto 0),
      mcdatain       => mcdata,
      mcms           => mcms_ctrl_s,
      mcrwn          => mcrwn,
      rst_mc_n       => RST_MC_N,
  ...
  rst_msix_i : reset_sync
    port map (
      RST_N     => rst_msix_n_s,
      CLK       => CLK_SYS,
      RST_OUT_N => rst_msix_sys_n_s
    );
  hsum_i : hsum
    generic map (
  ...
  mci_top_i : mci_top
    port map (
      ...
      ddr_2_resetn      => ddr_2_rst_n,
      msix_resetn       => rst_msix_n_s,
      ...
      ddr_3_resetn      => ddr_3_rst_n,
      ddr_0_resetn      => ddr_0_rst_n,
  ...
  msix_i : msix
    port map (
      ...
      CONV_DONE            => conv_done_s,
      HSUM_DONE            => hsum_done_s,
      MCADDR               => mcaddr(3 downto 0),
      MCDATAIN             => mcdata,
      MCMS                 => mcms_msix_s,
      MCRWN                => mcrwn,
      RST_MC_N             => RST_MC_N,
      RST_PCIE_N           => RST_PCIE_N,
      RST_SYS_N            => rst_msix_sys_n_s,
      USR_EVENT_MSIX_READY => USR_EVENT_MSIX_READY,
      MCDATAOUT            => mcdataout_msix_s,
