...
module altera_emif_avl_tg_driver # (
   ...
   altera_emif_avl_tg_read_compare # (
      ...
      .ecc_rdata_error                (ctrl_ecc_readdataerror),
      .ecc_user_interrupt             (ctrl_ecc_user_interrupt),
      .read_compare_fifo_full         (read_compare_fifo_full),
      .read_compare_fifo_empty        (read_compare_fifo_empty),
...
`ifdef ALTERA_EMIF_ENABLE_ISSP
   ...
   logic [3:0]  r_avl_ready;
   logic [3:0]  r_avl_read_req;
   logic [3:0]  r_avl_write_req;

   always_ff @(posedge clk)
   begin
      r_avl_ready[3:0]     <= {r_avl_ready[2:0],     avl_ready};
      r_avl_read_req[3:0]  <= {r_avl_read_req[2:0],  avl_read_req};
      r_avl_write_req[3:0] <= {r_avl_write_req[2:0], avl_write_req};
   end
   ...
   begin
      ...
      else if ((r_avl_read_req[3] | r_avl_write_req[3]) & r_avl_ready[3])
      begin
   ...
   begin
      ...
      else if (first_req_received)
      begin
         if (r_avl_read_req[3] & r_avl_ready[3])
         begin
            avl_byte_cnt <= avl_byte_cnt + avl_size;
         end
         else if(r_avl_write_req[3] & r_avl_ready[3])
         begin
