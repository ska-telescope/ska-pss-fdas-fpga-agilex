...
module altera_emif_avl_tg_burst_boundary_addr_gen # (
   ...
   parameter longint AMM_WORD_ADDRESS_DIVISIBLE_BY  = 1,
   ...
   generate
      ...
      begin: amm_word_address_divisible_by_gt_one
         ...
         initial
         begin
            assert ($size(AMM_WORD_ADDRESS_DIVISIBLE_BY) >= ADDR_WIDTH)
              else $error ("TG_BURST_BOUNDARY_ADDR_GEN__WIDTH_MISMATCH: ADDR_WIDTH should be less than size of AMM_WORD_ADDRESS_DIVISIBLE_BY");
         end
      end else
