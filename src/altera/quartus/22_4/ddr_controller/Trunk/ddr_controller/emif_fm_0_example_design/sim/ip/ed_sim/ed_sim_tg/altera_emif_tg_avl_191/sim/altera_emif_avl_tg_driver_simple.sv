...
module altera_emif_avl_tg_driver_simple # (
   ...
   state_t                              nxt_state /* synthesis ignore_power_up */;
...
`endif
   ...
   initial
   begin
      assert ($size(AMM_WORD_ADDRESS_DIVISIBLE_BY) >= TG_AVL_WORD_ADDR_WIDTH)
        else $error ("TG_DRIVER_SIMPLE__WIDTH_MISMATCH: TG_AVL_WORD_ADDR_WIDTH should be less than size of AMM_WORD_ADDRESS_DIVISIBLE_BY");
   end
