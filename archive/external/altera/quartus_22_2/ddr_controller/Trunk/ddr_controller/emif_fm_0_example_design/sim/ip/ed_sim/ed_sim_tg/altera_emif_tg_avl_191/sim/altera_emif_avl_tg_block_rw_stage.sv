...
module altera_emif_avl_tg_block_rw_stage # (
   ...
   addr_gen_select_t addr_gen_select_r   /* synthesis ignore_power_up */;
   addr_gen_select_t addr_gen_select_int /* synthesis ignore_power_up */;
   logic do_wait;
   ...
   begin
      if (!reset_n) begin
         ...
         if (SEQ_ADDR_COUNT > 0)
            addr_gen_select_int <= SEQ;
         else if (SEQ_ADDR_COUNT + RAND_ADDR_COUNT > 0)
            addr_gen_select_int <= RAND;
         else
            addr_gen_select_int <= RAND_SEQ;
         addr_gen_select_r <= SEQ;
         do_wait <= 1'b0;
         state <= INIT;

      end else begin
         addr_gen_select_r <= addr_gen_select_int;
         ...
         case (state)
            ...
            BLOCK_READ:
            begin
               if (addr_gen_select_r != addr_gen_select_int)
                  do_wait <= 1'b1;
               ...
               if (can_read) begin
                  if (block_size_counter == BLOCK_SIZE[BLOCK_SIZE_COUNTER_WIDTH-1:0] - 1'b1) begin
                     ...
                     if (block_write_counter == NUM_BLOCK_WRITES)
                        ...
                        state <= WAIT;
                     else if (USE_BLOCKING_ADDRESS_GENERATION && (addr_gen_select_r != addr_gen_select_int || do_wait))
                        state <= WAIT_ADDR_GEN;
                     else
                        state <= BLOCK_WRITE;
                  end else begin
   ...
   assign addr_gen_select = addr_gen_select_int;
endmodule
