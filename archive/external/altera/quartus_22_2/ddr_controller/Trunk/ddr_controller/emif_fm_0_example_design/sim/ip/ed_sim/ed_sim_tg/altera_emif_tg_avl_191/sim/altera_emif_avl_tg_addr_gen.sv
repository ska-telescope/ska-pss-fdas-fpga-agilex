...
module altera_emif_avl_tg_addr_gen # (
   ...
   // address mapping scheme explaination: 
   //  1. USE_UNIX_ID is a 3-bit ID to divide the address space for 8 AMM ports so that they don't overlap 
   //  2. the 1'b0 after the USE_UNIX_ID is to accomodate the overflow bit from addr_no_id
   //     when doing burst transaction at the address around addr_no_id = 17'hFFFF
   //  3. the random address generator use two LFSRs to generate the high 7 bits and low 9 bits of the random
   //     address separately, and form the final addr_no_id[16:0] = {rand_high[6:0], 1'b0, rand_low[8:0]}. This
   //     prevents two consecutive random addresses from overlapping with each other, since rand_high will not repeat 
   //     in short distance, say 16 or 32, which is the BLOCK RW size
